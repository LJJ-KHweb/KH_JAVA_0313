...
public class Run {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 추상적자료형(Abstract Data Type) -> interface + 구현체 -> Class + 알고리즘 -> method 합친게
		 * JAVA의 Collection
		 * 
		 * 배열과 컬렉션의 차리점 1. 배열의 사용하려고 하면 무조건 크기 할당을 해줘야함 -> 한번 지정한 크기는 못바꿈 2. 배열 중간 위치에
		 * 새로운 요소를 추가하거나 삭제할 때 기존값을 밀거나 떙기는 코드를 써야함
		 * 
		 * - 컬렉션 1. 크기의 제약이 자유로움 -> 처음 컬렉션을 사용할 때 크기를 지정하지 않아도 사용할 수 있음 -> 만약 요소가 추가되면서
		 * 크기를 넘어서는 상황이 발생하면 알아서 확장됨 2. 중간에 요소를 추가하거나 삭제하는 경우 기존값을 밀거나 땡기는 코드가 이미 구현되어있음
		 * -> 그때그때 필요한 메소드를 호출해서 사용하면됨
		 * 
		 * -중립 -배열의 경우 같은 타입의 자료형만 사용할 수있음 -컬렉션의 경우 여러 타입의 데이터들을 사용할 수 있음 : 같은 타입의 데이터만
		 * 묶어서도 사용가능
		 * 
		 * 사용할 데이터의 개수가 명확히 정해져 있고, 조회의 목적 -> 배열 데이터들의 추가, 수정, 삭제가 빈번하게 일어난다 -> 컬렉션
		 * 
		 * 개념적인 컬렌션의 종류
		 * 
		 * -List 계열 : 다루려고하는 값(Value)만 저장 / 저장 시 순서 유지(Index), 중복값 허용 -> LinkedList,
		 * ArrayList, Vector ... -set 계열 : 다루려고하는 값(Value)만 저장/ 저장 시 순서 유지 X, 중복값 허용X
		 * ->TreeSet, HashSet... -Map 계열 : 키(key) - 값(Value) 세트로 저장/ 저장 시 순서 유지 X, 키 중복
		 * X, 값 중복 O ->HashMap, TreeMap, MultiValueMap...
		 */
		Date now = new Date();
		...
		ArrayList<Object> plants = new ArrayList(3); // 3칸짜리 리스트를 구현하겠다
		...
		// add 메소드 오버로딩 엄청 잘돼있음
		// add(int index, E e) : List의 index에 전달할 요소를 추가
		//plants.add(0, "식물 시작!");
		plants.add(3, new Plant("관엽", "야자수"));
		System.out.println(plants);
		//	2. 요소를 수정하는 메소드
		//	
		plants.set(1, new Plant("plantss","asd"));
		System.out.println(plants);
		//	3. 요소 삭제
		//	remove(int index) : List 의 index 위치에 담긴 요소를 삭제하는 메소드
		plants.remove(3);
		System.out.println(plants);
		//IndexOutOfException == index값을 잘 고려해서 인자값을 전달해야함
		...
		// 4. 리스트의 크기를 반환 받기
		// size() 리스트의 담겨 있는 요소의 개수
		System.out.println(plants.size());
		// 5. List의 해당 index 위치에 담긴 요소를 반환받는 메소드
		// get(int index) : E
		System.out.println(plants);
		Plant p = (Plant)plants.get(2);
		...
		for(Object plant : plants) {
			System.out.println(plant);	//동적 바인딩
		}
		// 6.리스트 자르기
		// subList(int index1, int index2) :반환 List
		// index1 부터 index2까지의 요소를 추출해서 새로운 리스트 반환
