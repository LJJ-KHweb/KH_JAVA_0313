...
//목표 : 축구선수 CRUD(Create Read Update Delete) 구현 
// Create(생성) 	: View 에서 만들어진 데이터를 축구선수모양으로 만들고 저장
// Read(조횡)		: 저장되어있는 축구선수들을 조회해서 출력 
// Update(수정)	: 저장되어있는 축구선수들의 데이터를 갱신
// Delete(삭제)	: 저장되어있는 축구선수들의 데이터를 삭제

//공지사항 ( C : 계시글 작성 R : 게시판 U : 게시글 수정 D : 게시글 삭제)
//회원 (C : 회원가입, R : 마이페이지(내정보조회), U : 내정보수정, D : 회원탈퇴)

// 회원의 C를 구현해야겠다.
// 아이디, 비밀번호, 전화번호, 이메일
// 공지사항의 C를 구현해야겠다
// 게시글 제목, 게시글 내용, 게시글 작성자
...
public class Run {
	...
	public static void main(String[] args) {
		...
		// Model 에서 일어난 처리작없 결과를 View 로 전달